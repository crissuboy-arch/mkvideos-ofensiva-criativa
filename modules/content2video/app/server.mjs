...
// [MKVIDEOS PATCH] A UI estática empacotada do upstream fica desativada por padrão.
// O MKVideos consome só a API JSON deste motor e apresenta tudo pela própria interface.
// Defina C2V_ALLOW_BUNDLED_UI=1 apenas para depurar o upstream isoladamente.
const ALLOW_BUNDLED_UI = process.env.C2V_ALLOW_BUNDLED_UI === "1";
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    // [MKVIDEOS PATCH] readiness leve (não dispara `codex login status`).
    if (req.method === "GET" && (pathname === "/healthz" || pathname === "/api/healthz")) {
      return json(res, 200, { ok: true, version: formatDisplayVersion(PACKAGE.version), bundledUi: ALLOW_BUNDLED_UI });
    }
    ...
    // [MKVIDEOS PATCH] UI estática empacotada só quando explicitamente liberada.
    if (req.method === "GET" && ALLOW_BUNDLED_UI) {
    ...
    if (req.method === "GET" && (pathname === "/" || pathname === "/index.html")) {
      return json(res, 404, {
        error: "A interface empacotada do Content2Video está desativada. Use o painel do MKVideos (aba “URL → Vídeo”) ou o comando `mkivideos url2video`.",
      });
    }
